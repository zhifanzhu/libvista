...
from libvista.odlib import box_utils
...
class BoxUitlsTest(unittest.TestCase):
    ...
    def test_xyxy_and_xywh(self):
        ...
        torch.testing.assert_close(box_utils.xyxy_to_xywh(box_1d_xyxy_th), box_1d_xywh_th)

        box_3d_xywh_th = torch.as_tensor(box_3d_xywh)
        box_3d_xyxy_th = torch.as_tensor(box_3d_xyxy)
        torch.testing.assert_close(box_utils.xywh_to_xyxy(box_3d_xywh_th), box_3d_xyxy_th)
